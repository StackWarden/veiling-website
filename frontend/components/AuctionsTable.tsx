...
export default function AuctionTable({ initialAuctions }: { initialAuctions?: Auction[] }) {
  const [auctions, setAuctions] = useState<Auction[]>(initialAuctions ?? []);
  const { data, loading, error } = useGet<Auction>({ route: "/auctions" });
        ...
        <table className="w-full text-center">
          ...
          <tbody>
            {auctions.map((a) => {
              ...
              const trClass = `cursor-pointer ${isSelected ? "bg-black text-white" : "hover:bg-gray-300"} rounded-full px-2 py-1`;
              return (
              <tr key={a.id} 
                role="button"
                tabIndex={0}
